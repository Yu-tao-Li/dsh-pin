...
- **两级置顶**——📌 把会话置顶到**本工作区**内(持久顺序,全局生效);⌃⌃ 把会话**置顶到所有工作区最上面**:会话进入侧边栏顶部的全局置顶区(悬浮在所有工作区组之上,组内原行隐藏),点击置顶区里的行直接打开该会话。工作区组头部另有 ⌃⌃ 按钮,可单独把整个工作区置顶。
...
- **全局置顶是每浏览器生效**(DSH 宿主没有“全局置顶会话”的概念);工作区内置顶与整工作区置顶是全局持久的。
