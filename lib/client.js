 ...
 *
 * Record-based toggle like sessions: a recorded workspace unpins wherever
 * it currently sits; an unrecorded first workspace pins in place (no move).
 *
 * @returns one of:
 *  - { kind: "unsupported" }
 *  - { kind: "pin", before, record }      insertBefore(wid, before); before null = in place
...
function planWorkspacePin(state, workspaceId) {
	...
	return { kind: "pin", before: i === 0 ? null : ids[0], record: positionOf(ids, workspaceId) };
}
